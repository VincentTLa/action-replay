...
- **No foreground service → save-on-background instead (2026-06-22):** backgrounding still stops capture (no camera-FGS), but a battle in progress is now **saved, not discarded**. A shared `saveAndTeardown` (in `CameraScreen`) runs on BOTH exit paths — `surfaceDestroyed` and the `DisposableEffect` `onDispose` — calling `engine.endSession()` (no-op if none) before `stop()`, and clearing inline-playback state. Manifest `configChanges` was widened to `…|uiMode|density|smallestScreenSize|screenLayout` so cosmetic/resize changes (dark mode, battery saver) don't recreate the Activity mid-battle. **Accepted limit:** a hard process kill (swipe from recents) runs neither path → that battle is lost; surviving a kill needs a real camera foreground service (deliberately out of scope).
...
- **Two playback paths by design (2026-06-22):** the **auto-rewind** inline replay stays on `VideoView` (no dep, the heavily-debugged transition/skip/watchdog path — don't migrate it). The **reel rewatch** uses Media3 **ExoPlayer** (`ScrubberViewer`) because a frame-accurate scrubber needs `SeekParameters.EXACT`, which `VideoView`/`MediaPlayer` can't do reliably. Frame-step uses ±33ms (≈1 frame @30fps) seeks; accumulates ~1 frame of rounding drift over ~30 steps (acceptable). The viewer pauses on `ON_STOP` via the Activity's `Lifecycle` (lifecycle-runtime-ktx, no new artifact). ponytail: the player isn't auto-resumed on return — user re-taps PLAY.
